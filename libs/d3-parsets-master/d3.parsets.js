...
(function() {
  d3.parsets = function() {
    var event = d3.dispatch("sortDimensions", "sortCategories"),
        ...
        tension0,
        duration = 500,
        setDimensionsWidth_ = function(c, d, w, totalNum) {
          console.log(c.name, d.name);
          return c.count / totalNum * w;
        };
    ...
    function parsets(selection) {
      selection.each(function(data, i) {
        ...
        function updateRibbons() {
          ...
          mouse.enter().append("path")
              ...
              .on("click.parsets", function(d) {
                cancelEvent();
                console.log("hi there click on ribbon")
                console.log(d);
                ribbon.classed("active", false);
                ribbon.classed("pinned", false);
                if (dragging) return;
                highlight_pin(d, true);
                showTooltip(tooltip_.call(this, d));
              });
          mouse
        ...
        // Highlight a node and its descendants, and optionally its ancestors.
        function highlight_pin(d, ancestors) {
          if (dragging) return;
          var highlight = [];
          (function recurse(d) {
            highlight.push(d);
        ...
        function updateCategories(g) {
          ...
          category
              ...
              .on("mouseout.parsets", function() {
                unhighlight();
              })
              // .on("mousedown.parsets", function() {
              //   console.log("hi there! mousedown on bar")
              //   cancelEvent();
              // })
              .on("click.parsets", function(d) {
                cancelEvent();
                console.log("hi there! click on bar")
                console.log(d)
                ribbon.classed("active", false);
                if (dragging) return;
                d.nodes.forEach(function(d) { highlight_pin(d, true); });
                showTooltip(categoryTooltip.call(this, d));
              })
              .call(d3.behavior.drag()
                .origin(identity)
          ...
          categoryEnter.append("rect")
              .attr("width", function(d) { console.log("categoryEnter: "+d); return d.dx; })
              .attr("y", -20)
              .attr("height", 20);
    ...
    parsets.setDimensionsWidth = function(_) {
      if (!arguments.length) return setDimensionsWidth_;
      setDimensionsWidth_ = _;
      return parsets;
    }
    ...
    function layout(tree, dimensions, ordinal) {
      ...
      dimensions.forEach(function(d) {
        ...
        d.categories.forEach(function(c) {
          ...
          c.percent = c.count / total;
          c.x = x;
          c.dx = setDimensionsWidth_(c, d, (width - spacing), total);
          c.in = {dx: 0};
          c.out = {dx: 0};
          x += c.dx + p;
        });
